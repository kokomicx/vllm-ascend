...
def test_v2_methods_exist():
    ...
    assert hasattr(NPUModelRunner, "_get_layer_attention_backends"), (
        "_get_layer_attention_backends missing"
    )
...
def test_gate_default_off():
    ...
    assert VLLM_ASCEND_USE_KV_LAYOUT_DISPATCH is False, "Gate should be OFF by default"
...
def test_split_sizes_sums():
    ...
    for name, layout, spec, kw in cases:
        ...
        assert len(sizes) == layout.num_tensors(), f"{name}: expected {layout.num_tensors()} sizes, got {len(sizes)}"
        assert abs(sum(sizes) - total) < layout.num_tensors(), f"{name}: sum={sum(sizes)} != total={total}"
        print(f"  ✓ {name}: {sizes}")
