...
def test_split_kv_layout_splits_and_reshapes_distinct_fields():
    layout = SplitKVLayout(
        ...
        key_split_factor=1.5,
        value_split_factor=3.0,
    )
    total_size = 2 * 16 * 8 * (64 + 32) * torch.empty((), dtype=torch.float16).element_size()
    ...
    key_cache, value_cache = layout.reshape(key_raw, value_raw, (2, 2, 16, 8, 64))

    assert key_size + value_size == total_size
    assert key_cache.shape == (2, 16, 8, 64)
    assert value_cache.shape == (2, 16, 8, 32)
